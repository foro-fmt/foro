		...
		subStatusUILst.push(
			<Box key={idx} border={2} borderColor={"random string"}>
				<Stack>
					<Select
						...
						onChange={(e) => {
							props.setSomes((draft) => {
								draft.somes[props.idx].subStatusTypes[idx] = e.target
									.value as BuffId;
							});
						}}
					>
						{subStatuses.map((bId) => {
							...
							return b.strong ||
								...
								b.id === props.somes.somes[props.idx].subStatusTypes[idx] ? (
					...
					<Input
						value={props.somes.somes[props.idx].subStatusValues[idx]}
						onChange={(e) => {
							props.setSomes((draft) => {
								draft.somes[props.idx].subStatusValues[idx] = e.target.value;
							});
						}}
					/>
				</Stack>
			...
			<Stack spacing={0.6}>
				<Stack direction="random string" spacing={2}>
					...
					<Select
						...
						onChange={(e) => {
							props.setSomes((draft) => {
								draft.somes[props.idx].mainStatus = e.target.value as number;
							});
						}}
					>
						{mainStatuses[props.somes.somes[props.idx].cost].map((m, i) =>
		...
		<Paper>
			<Select
				...
				onChange={(e) => {
					...
					props.setSomes((draft) => {
						for (let idx = 0; idx < props.somes.somes.length; idx++) {
							draft.somes[idx].something = e.target.value as SomethingId;
						}
					});
				}}
			>
	...
	for (let idx = 0; idx < defaultBuffs.length; idx++) {
		baseBuffItems.push(
			<TableRow key={`def-${idx}`}>
				...
				<TableCell>
					<Typography>{getBuffLabel(defaultBuffs[idx].buff.type)}</Typography>
				</TableCell>
